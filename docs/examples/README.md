...
The simplest possible initramfs image with only busybox and kestrel agent. Perfect for learning or as a starting point.
...
Nginx web server converted from the official Alpine image. Shows basic OCI to image conversion.

**Size:** ~50-60 MB
**Best for:** Static sites, reverse proxy, load balancing

### [oci-postgres.toml](oci-postgres.toml)
PostgreSQL database image. Demonstrates handling larger images with database software.
...
1. **Copy the example** to your project directory:
   ```bash
   cp docs/examples/initramfs-go-webserver.toml my-image/fledge.toml
   ```
...
### For OCI Rootfs Images

- **Choose Alpine variants**: Smaller base images = smaller images
