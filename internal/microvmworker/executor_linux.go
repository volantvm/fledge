...
func buildInitScript(process executor.ProcessInfo) string {
	...
	buf.WriteString("\t\t\tlog_console \"microvm init: ifconfig $iface up succeeded\"\n")
	buf.WriteString("\t\t\treturn 0\n")
	buf.WriteString("\t\telse\n")
	...
	buf.WriteString("\t\t\tif ! wait_iface_ready \"$iface\"; then\n")
	buf.WriteString("\t\t\t\tlog_console \"microvm init: link did not become ready on $iface; retrying\"\n")
	buf.WriteString("\t\t\t\tcontinue\n")
	buf.WriteString("\t\t\tfi\n")
